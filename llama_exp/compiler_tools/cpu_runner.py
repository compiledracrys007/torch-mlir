...
from torch_mlir.compiler_utils import run_pipeline_with_repro_report, OutputType
import os
import ctypes
import numpy as np
...
def compile_and_run(model, inputs):
    imported_model = fx.export_and_import(model, inputs, enable_ir_printing=os.getenv("PFA"), 
    ...
    run_pipeline_with_repro_report(
            ...
            enable_ir_printing=os.getenv("PFA"))
    
    jit_module = CpuBackendInvoker(imported_model)

    numpy_inputs = [inp.numpy() for inp in inputs]
    outputs = jit_module.main(*numpy_inputs)
    if len(outputs) == 1:
